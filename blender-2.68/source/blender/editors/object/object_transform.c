...
static int object_freeze_transform_apply_exec(bContext *C, wmOperator *op)
{
	...
	if (loc || rot || sca) {
		// apply_objects_internal(C, op->reports, loc, rot, sca);
	CTX_DATA_BEGIN (C, Object *, ob, selected_editable_objects)
	{
		add_v3_v3(ob->dloc, ob->loc);
		ob->loc[0] = ob->loc[1] = ob->loc[2] = 0.0f;
		if(( ob->type != OB_MESH)&&(ob->type != OB_ARMATURE)&&(ob->type != OB_LATTICE)&&(ob->type != OB_MBALL)&&(ob->type != OB_CURVE)&&(ob->type != OB_SURF))
		{
			if (ob->size[0] != 1.0f)
				ob->dscale[0] = ob->dscale[0] + (ob->size[0] - 1) * ob->dscale[0];
			if (ob->size[1] != 1.0f)
				ob->dscale[1] = ob->dscale[1] + (ob->size[1] - 1) * ob->dscale[1];
			if (ob->size[2] != 1.0f)
				ob->dscale[2] = ob->dscale[2] + (ob->size[2] - 1) * ob->dscale[2];
			ob->size[0] = ob->size[1] = ob->size[2] = 1.0f;
			add_v3_v3(ob->dquat, ob->quat);
			add_v3_v3(ob->drot, ob->rot);
			add_v3_v3(ob->drotAxis, &ob->rotAngle);
			zero_v3(ob->rot);
			unit_qt(ob->quat);
			unit_axis_angle(ob->rotAxis, &ob->rotAngle);
		}
...
void OBJECT_OT_freeze_transform_apply(wmOperatorType *ot)
{
	/* identifiers */
	ot->name = "Apply freezz Object Transform";
	ot->description = "Appsdds transformation to its data";
	ot->idname = "OBJECT_OT_freeze_transform_apply";

	/* api callbacks */
	ot->exec = object_freeze_transform_apply_exec;
