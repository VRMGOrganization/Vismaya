...
static void ui_theme_init_new(bTheme *btheme)
{
	...
	ui_theme_init_new_do(&btheme->tlayer);
...
//	rgba_char_args_set(btheme->tbuts.panel, 0x82, 0x82, 0x82, 255);
	...
	/* space layers */
	/* to have something initialized */
	btheme->tlayer = btheme->tv3d;

	rgba_char_args_set_fl(btheme->tlayer.back,   0.45, 0.45, 0.45, 1.0);
	//rgba_char_args_set(btheme->tlayer.panel, 0x82, 0x82, 0x82, 255);
...
/* TEMPORAL, remove me! (ton) */				
	...
	if (bmain->versionfile < 265 || (bmain->versionfile == 265 && bmain->subversionfile < 2)) {
		...
		for (btheme = U.themes.first; btheme; btheme = btheme->next) {
			...
			btheme->tlayer.panelcolors = btheme->tui.panel;
