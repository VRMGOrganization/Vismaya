...
EnumPropertyItem space_type_items[] = {
	...
	{SPACE_LAYER, "LAYER_EDITOR", ICON_LAYER_MANAGE, "Layer Management", ""},
	{0, NULL, 0, NULL, NULL}
};
...
static void rna_SpaceLayer_lock_camera_and_layers_set(PointerRNA *ptr, int value)
{
	SpaceLayer *slayer = (SpaceLayer *)ptr->data;
	bScreen *sc = (bScreen *)ptr->id.data;

	slayer->scenelock = value;

	if (value) {
		int bit;
		slayer->lay = sc->scene->lay;
		// seek for layact 
		bit = 0;
		while (bit < 32) {
			if (slayer->lay & (1 << bit)) {
		...
		slayer->camera = sc->scene->camera;
	}
}

static void rna_SpaceLayer_view3d_update(Main *bmain, SpaceLayer *UNUSED(slayer_unused), PointerRNA *ptr)
{
	SpaceLayer *slayer = (SpaceLayer *)ptr->data;

	BKE_screen_view3d_main_sync(&bmain->screen, slayer);
}

static void rna_SpaceLayer_layer_set(PointerRNA *ptr, const int *values)
{
	SpaceLayer *slayer = (SpaceLayer *)(ptr->data);
	
	slayer->lay = ED_view3d_scene_layer_set(slayer->lay, values, &slayer->layact);
}

static void rna_SpaceLayer_layer_update(Main *bmain, SpaceLayer *slayer, PointerRNA *ptr)
{
    rna_SpaceLayer_view3d_update(bmain, slayer, ptr);
...
static void rna_def_space_layer(BlenderRNA *brna)
{
	StructRNA *srna;
	PropertyRNA *prop;

	srna = RNA_def_struct(brna, "SpaceLayerEditor", "Space");
	RNA_def_struct_sdna(srna, "SpaceLayer");
	RNA_def_struct_ui_text(srna, "Space Layer Editor", "Layer editor space data");
	
	
	prop = RNA_def_property(srna, "lock_camera_and_layers", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "scenelock", 1);
	RNA_def_property_boolean_funcs(prop, NULL, "rna_SpaceLayer_lock_camera_and_layers_set");
	RNA_def_property_ui_text(prop, "Lock Camera and Layers",
	                         "Use the scene's active camera and layers in this view, rather than local layers");
	RNA_def_property_ui_icon(prop, ICON_LOCKVIEW_OFF, 1);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
	...
	RNA_def_property_boolean_sdna(prop, NULL, "lay", 1);
	RNA_def_property_array(prop, 20);
	RNA_def_property_boolean_funcs(prop, NULL, "rna_SpaceLayer_layer_set");
	RNA_def_property_ui_text(prop, "Visible Layers", "Layers visible in this LayerManager");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, "rna_SpaceLayer_layer_update");
