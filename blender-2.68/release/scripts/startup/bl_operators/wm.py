...
class WM_OT_custom_menu(Operator):    
    bl_idname = "wm.custom_menu"
      ...
      try:
        ...
        if os.path.isfile(path):  
          ...
          self.custom(custom)
            ...
            self.report({'INFO'}, "No custom menu in custom tool")
            return {'FINISHED'}
      return {'FINISHED'}
class WM_OT_custom_clear_menu(Operator):    
    bl_idname = "wm.custom_clear_menu"
    bl_label = "Custom Menu"    
    def execute(self, context):
        try:
         global custom  
...
class WM_OT_custom_remove_menu(Operator):
      bl_idname = "wm.custom_remove_menu"
      bl_label = "Custom Menu" 
      doc_id = doc_id   
      def execute(self, context):
        s = 0
          ...
          for a in n:
               if a != c_menu:
                 if custom_remove == "":
                    ...
                    custom_remove = custom_remove +","+ a
                 custom = custom_remove
                 s = 1
          if s == 0:
            custom = ""
          f = open(path,'w')
