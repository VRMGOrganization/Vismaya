...
class PieMenuSettings(bpy.types.PropertyGroup):
    ...
    def register(cls):
        ...
        cls.pieThetaShift = FloatProperty(
...
class piemenus_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        box.prop(scn.piemenus,'PiePrefs')
        if scn.piemenus.PiePrefs:
            boxrow=box.row()
            ...
            boxrow=box.row()
            boxrow.prop(scn.piemenus, 'clockBool')
            
            boxrow=box.row()
            boxrow.prop(scn.piemenus, 'pieOuterRadius')
            
            boxrow=box.row()
            boxrow.prop(scn.piemenus, 'double_size')
            
            boxrow=box.row()
            boxrow.prop(scn.piemenus, 'pieRobustCleanup')
            
            boxrow=box.row()
            boxrow.prop(scn.piemenus, 'pieBorderDelay')
            
            boxrow=box.row()
            boxrow.prop(scn.piemenus, 'pieSquish')
            
            boxrow=box.row()
            boxrow.prop(scn.piemenus, 'pieDiamond')
            
            boxrow=box.row()
            boxrow.prop(scn.piemenus, 'pieThetaShift')
...
class OBJECT_OT_ActivatePieButton(bpy.types.Operator):
    ...
    def execute(self, context):
        if len(context.scene.pie_settings) != 0:
...
def handler_post_load(dummy):
    ...
    if addon_utils.check("piemenus")[1]:
        ...
        scn.piemenus.pieImageLoadedbpy = False
...
def handler_pre_save(dummy):
    ...
    scn = bpy.context.scene
    if  scn.piemenus.pieRobustCleanup:
        img = bpy.data.images.get("blender_icons_x2.png")
        if img:
            img.user_clear()
